...
1. `Web-2-Notion/popup/static/js/main.js` — `tableToList` (popup-side table rule)
...
1. `Web-2-Notion/popup/static/js/main.js` — `CN` image case (inline-image conversion)
...
1. `Web-2-Notion/popup/static/js/main.js` — `SN` paragraph post-processor
...
1. Legacy / scattered producers (tests, helpers, older code paths)
...
[Untitled](https://www.notion.so/301a89fedba58075b272e3fe578bcee7?pvs=21)
