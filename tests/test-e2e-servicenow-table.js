...
console.log('🧪 test-e2e-servicenow-table — full pipeline (content-script -> popup)');
...
function sanitizeCell(cell) {
  ...
  const id = 'CELL_E2E_' + Math.random().toString(36).substr(2, 6).toUpperCase();
  window.__TABLE_CELL_CONTENT_MAP__[id] = {
    paragraphs: textWithNewlines.split('\n'),
    flattened: textWithNewlines.split('\n').join(' '),
    meta: { containsImage: images.length > 0, hasLinks: false, approxLength: (textWithNewlines || '').length },
  };
...
const flattened = [marker0, marker1].map((m) => {
