...
const patch = require(path.join(__dirname, "..", "Web-2-Notion", "popup", "shim", "table-to-list-patch.js"));

console.log("🧪 test-no-duplicate-dataurl-when-xcellidx — ensure XCELLIDX prevents extra data: URL markdown");
...
const html = `<table><tr><td>XCELLIDX(CELL_123)XCELLIDX<img data-original-src="data:image/png;base64,AAA" data-stn-preserve="1" alt="Img"></td></tr></table>`;
const pre = conv(html);

console.log("Preprocessed ->", pre.replace(/\s+/g, " ").substring(0, 200));

if (!/XCELLIDX\(CELL_123\)XCELLIDX/.test(pre)) {
...
if (/!\[.*\]\(data:image\//.test(pre)) {
  console.error("❌ Preprocessor injected data:image markdown where XCELLIDX existed");
  process.exit(1);
}
