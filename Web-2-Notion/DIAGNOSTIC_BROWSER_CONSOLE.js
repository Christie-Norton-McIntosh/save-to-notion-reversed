...
if (typeof window.__imageUrlArray !== "undefined") {
  ...
  console.log(
    "   ❌ Extension globals NOT found - extension may not be loaded",
  );
}
...
imagesInAnchors.forEach((img, i) => {
  ...
  console.log(
    `       src type: ${src.startsWith("data:") ? "BASE64" : src.startsWith("http") ? "HTTP" : "OTHER"}`,
  );

  if (src.startsWith("data:") && anchorHref.includes("/viewer/attachment/")) {
    console.log(
      `       ⚠️  This is a BASE64 image with viewer/attachment anchor`,
    );
    console.log(
      `       ⚠️  Extension should use anchor href: ${anchorHref.substring(0, 100)}`,
    );
  }
});
...
if (tables.length > 0) {
  ...
  if (firstCell) {
    ...
    console.log(
      `   - Text content: "${text.substring(0, 150).replace(/\s+/g, " ")}..."`,
    );
...
console.log(
  "   - Extension loaded:",
  typeof window.__imageUrlArray !== "undefined" ? "✅" : "❌",
);
