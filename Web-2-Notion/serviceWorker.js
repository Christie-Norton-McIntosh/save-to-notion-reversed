...
// Crash guards: keep the worker alive by catching top-level errors
const Dn = { last: 0, count: 0, max: 5, windowMs: 1e4 };
function guardLog(e, t) {
  const n = Date.now();
  n - Dn.last > Dn.windowMs && (Dn.count = 0);
  Dn.last = n;
  if (Dn.count < Dn.max) {
    console.error(`[ServiceWorker][Guard] ${e}:`, t);
    t && t.stack && console.error(t.stack);
  }
  Dn.count++;
}
typeof self != "undefined" &&
  (self.addEventListener("unhandledrejection", (e) => {
    (guardLog("unhandledrejection", e == null ? void 0 : e.reason),
      e.preventDefault && e.preventDefault());
  }),
  self.addEventListener("error", (e) => {
    guardLog("error", (e == null ? void 0 : e.error) || e.message);
  }));
function xt(e) {
...
function yr(
  ...
  return {
    ...
    text: e.url
      ? [
          ["[🔗 link]", [["a", e.url]]],
          [` — ${((a = e.name) == null ? void 0 : a.trim()) ?? "Untitled"}`],
        ]
      : e.name,
    blockFormat: i,
    afterId: n.notionListAfterId,
...
const El = Object.freeze(
  ...
  Ol = {
    ...
    replaceDataUrlPlaceholders: async (e, t) => {
      ...
      try {
        // Basic validation to avoid Notion 400s on bad page IDs or empty data
        const pageId = (e.pageId || "").trim();
        const spaceId = (e.spaceId || "").trim();
        const placeholderMap = e.placeholderMap || {};

        if (!placeholderMap || Object.keys(placeholderMap).length === 0) {
          console.debug(
            "[ServiceWorker] No placeholders provided; skipping replacement",
          );
          return { success: false, error: "no-placeholders" };
        }

        if (!/^[a-f0-9]{32}$/i.test(pageId)) {
          console.debug(
            "[ServiceWorker] Invalid pageId; skipping placeholder replacement",
            pageId,
          );
        ...
        if (!spaceId) {
          console.debug(
            "[ServiceWorker] Missing spaceId; skipping placeholder replacement",
          );
        ...
        // Build replacement list and determine if we need to fetch blocks
        const replacements = [];

        // Prefer blockIds supplied by the popup (we inject them when creating
        // the placeholder child blocks). This avoids brittle text searches.
        ...
        for (const uniqueId in placeholderMap) {
          ...
          if (placeholder && placeholder.blockId) {
            console.log(
              "[ServiceWorker] Using provided blockId for placeholder:",
              uniqueId,
              "->",
              placeholder.blockId,
            );
        ...
        if (missingBlockIds.size > 0) {
          console.log(
            "[ServiceWorker] Need to locate",
            missingBlockIds.size,
            "placeholder blocks by text search",
          );
          ...
          blocks = pageData.recordMap.block;
          ...
          for (const blockId in blocks) {
            ...
            // Log every text block we check (helps debug in the field)
            if (block.type === "text") {
            ...
            if (
              ...
              for (const uniqueId of missingBlockIds) {
                console.log(
                ...
                if (blockText === uniqueId) {
                  ...
                  missingBlockIds.delete(uniqueId);
                  break;
                }
              }
            }
          ...
          console.log(
            "[ServiceWorker] All placeholders already have blockIds; skipping loadPageChunk",
          );
        }

        if (missingBlockIds.size > 0) {
          console.debug(
            "[ServiceWorker] WARNING: Still missing blockIds for placeholders:",
        ...
        if (replacements.length === 0) {
          console.debug(
            "[ServiceWorker] WARNING: No placeholder blocks found!",
          );
          console.debug(
...
Please try again
(no worry we won't read any of your emails 😊)`),
          { success: !1 }),
  },
  Bl = {
    ...
    new Promise(async (n) => {
      try {
        ...
        try {
          e({
            __bgAskSuccess: !1,
            __bgAskError: o == null ? void 0 : o.toString(),
          });
        } catch {}
        n(!1);
      }
...
function jl(e, t, n) {
  ...
  try {
    return e.v2
      ? e.destination != "background"
        ? e.destination === "content"
          ? (z(e.event, e.props, (o = t.tab) == null ? void 0 : o.id).then(n),
            !0)
          : Co(n, () => {
              var i;
    ...
    guardLog("runtime.onMessage", i);
    try {
      n &&
        n({ __bgAskSuccess: !1, __bgAskError: i == null ? void 0 : i.message });
    } catch {}
    return !1;
  }
}
...
async function Rl(e) {
  try {
        ...
        if (n.event == "payload") {
          ...
          (So.add(t),
            F[e.name](n.props, e.sender, {
              ...
              .catch((o) => {
                (e.postMessage({ event: "error", props: null }),
                  e.disconnect());
              }));
        }
      });
    ...
    guardLog("runtime.onConnect", t);
    try {
      e.postMessage({
        event: "error",
        props: { message: t == null ? void 0 : t.message },
      });
    } catch {}
    try {
      e.disconnect();
