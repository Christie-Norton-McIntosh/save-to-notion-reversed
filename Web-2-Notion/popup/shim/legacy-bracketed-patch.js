...
(function () {
  ...
  function extractLegacyPlaceholderAlt(text) {
    if (!text) return null;
    ...
    const withoutTrailingEmptyParens = trimmed.replace(/\(\s*\)$/, "").trim();
    if (withoutTrailingEmptyParens) {
      candidates.add(withoutTrailingEmptyParens);
    }
    ...
    if (trimmed.startsWith("(") && trimmed.endsWith(")")) {
      ...
      if (inner) {
        ...
        const innerWithoutTrailingEmptyParens = inner
          .replace(/\(\s*\)$/, "")
          .trim();
        if (innerWithoutTrailingEmptyParens) {
          candidates.add(innerWithoutTrailingEmptyParens);
        }
      }
    }
  ...
  function visitTableContexts(root, visitor) {
    ...
    if (root.querySelectorAll) {
      root
        .querySelectorAll(
          "table, tbody, tr, td, th, .table-cell, .stn-table-cell",
        )
        .forEach((node) => {
          if (isElement(node)) {
            visitor(node);
  ...
  function storePreservedImage(img) {
    ...
    const parentElement =
      img.parentNode.nodeType === Node.ELEMENT_NODE ? img.parentNode : null;
  ...
  function handleBracketedText(node, alt) {
    ...
    log(
      preserved
        ? "restoring preserved image from global observer"
        : "converting placeholder to bullet",
      alt,
    );
  ...
  function handleMutations(mutations) {
    ...
    mutations.forEach((mutation) => {
      ...
      nodes.forEach((node) => {
        ...
        const alt = extractLegacyPlaceholderAlt(text);
        if (!alt) return;
        ...
        const observer = new MutationObserver((mutations) => {
          mutations.forEach((mutation) => {
            Array.from(mutation.addedNodes || []).forEach((node) => {
              if (!node || node.nodeType !== Node.TEXT_NODE) return;
              const text = node.textContent || "";
              const placeholderAlt = extractLegacyPlaceholderAlt(text);
              if (!placeholderAlt || placeholderAlt !== alt) return;
